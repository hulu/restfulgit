...
class ConditionalGetTestCase(_RestfulGitTestCase):
    ...
    def test_cget_valid_etag(self):
        resp = self.client.get(self.url, headers={'If-None-Match': self.etag})
        self.assertStatus(resp, 304)

    def test_cget_invalid_etag(self):
        resp = self.client.get(self.url, headers={'If-None-Match': 'invalid etag :)'})
        # no match should result in content being served, not an error
        self.assert200(resp)
